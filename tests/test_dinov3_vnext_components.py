...
def test_mask_pooling_blend_alpha_controls_cls_vs_mask_mix():
    torch.manual_seed(0)
    cfg = get_config()
    cfg.MODEL.TYPE = "DINOv3MultiHead"
    cfg.MODEL.IN_CHANS = 3
    ...
    cfg.MODEL.DINOV3.PATCH_SIZE = 4
    cfg.MODEL.MASK_POOLING.ENABLED = True
    cfg.MODEL.MASK_POOLING.USE_CLS_FALLBACK = True
    cfg.MODEL.MIL.ENABLED = False
    ...
    images = torch.randn(2, 3, 16, 16)
    mask_weights = torch.rand(2, 16)

    with torch.no_grad():
        cls_tokens, _, _ = model.backbone(images)
        cls_pooled = cls_tokens.squeeze(1)

        model.config.MODEL.MASK_POOLING.BLEND_ALPHA = 1.0
        pooled_mask, _ = model._forward_single(images, None, None, mask_weights)

        model.config.MODEL.MASK_POOLING.BLEND_ALPHA = 0.0
        pooled_cls, _ = model._forward_single(images, None, None, mask_weights)
        assert torch.allclose(pooled_cls, cls_pooled, atol=1e-6, rtol=1e-6)

        model.config.MODEL.MASK_POOLING.BLEND_ALPHA = 0.5
        pooled_half, _ = model._forward_single(images, None, None, mask_weights)
        expected_half = 0.5 * pooled_mask + 0.5 * cls_pooled
        assert torch.allclose(pooled_half, expected_half, atol=1e-6, rtol=1e-6)
