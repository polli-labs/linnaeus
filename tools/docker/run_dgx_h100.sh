#!/bin/bash
# DGX H100 training wrapper script
...
# Default number of GPUs (H100 DGX typically has 8)
NPROC_PER_NODE="${NPROC_PER_NODE:-8}"

# Source environment variables from YAML file
echo "Loading environment variables for DGX H100..."

# Convert YAML to environment variables
ENV_FILE="$REPO_ROOT/configs/env_vars/dgx_h100.yaml"
if [ -f "$ENV_FILE" ]; then
...
echo ""
echo "Starting DGX H100 training with $NPROC_PER_NODE GPUs..."
...
# Run distributed training with DGX-optimized settings
