...
# Configuration
IMAGE_NAME="frontierkodiak/linnaeus:dev"
...
# Build the Docker image using BuildKit (with timeout and retry)
echo "Building Docker image..."
DOCKER_BUILDKIT=1 timeout 30000 docker build \
  --progress=plain \
  --pull \
  -t "$IMAGE_NAME" \
  -f "$DOCKERFILE_PATH" \
  "$TEMP_DIR" || {
    echo "Build failed or timed out. Retrying with smaller timeout..."
    DOCKER_BUILDKIT=1 timeout 12000 docker build \
      --no-cache \
      --progress=plain \
      -t "$IMAGE_NAME" \
      -f "$DOCKERFILE_PATH" \
