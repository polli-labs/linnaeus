    ...
    try:
        ...
        with _shutdown_lock:
            if _shutdown_in_progress:
                ...
                # TODO(B012): Returning from within a finally block can suppress exceptions that may have been raised earlier in the try block.
                # This is flagged by ruff (B012) because it can make debugging harder and lead to silent failures.
                # The logic here is intended to skip the rest of the cleanup if an emergency shutdown is already in progress,
                # but this should ideally be handled outside the finally block or with a different control flow.
                # Refactor this to avoid returning from finally, ensuring exceptions are not accidentally silenced.
                return
