...
from .basic_loss import CrossEntropyLoss, LabelSmoothingCrossEntropy, SoftTargetCrossEntropy
...
def get_task_specific_config(val_or_list, task_keys: list[str], param_name: str = "") -> list:
    """
    ...
    if isinstance(val_or_list, list):
        if len(val_or_list) == len(task_keys):
            ...
            raise ValueError(f"{param_name} must match number of tasks. Expected {len(task_keys)}, got {len(val_or_list)}")
    else:
...
def prepare_loss_functions(
    ...
    funcs_train = get_task_specific_config(config.LOSS.TASK_SPECIFIC.TRAIN.FUNCS, task_keys, "TRAIN.FUNCS")
    funcs_val = get_task_specific_config(config.LOSS.TASK_SPECIFIC.VAL.FUNCS, task_keys, "VAL.FUNCS")

    logger.info(f"Training loss functions requested: {dict(zip(task_keys, funcs_train, strict=False))}")
    logger.info(f"Validation loss functions requested: {dict(zip(task_keys, funcs_val, strict=False))}")

    logger.info("Calculating class weights")
    # Ensure splits exist before accessing
    class_weights_train = calculate_class_weights(class_label_counts.get("train", {}), config)
    class_weights_val = calculate_class_weights(class_label_counts.get("val", {}), config)

    # Determine ignore_index based on phase and config
    # Only ignore index 0 (null) if PHASE1_MASK_NULL_LOSS is True
    ignore_idx_val = None
    if hasattr(config.TRAIN, "PHASE1_MASK_NULL_LOSS") and config.TRAIN.PHASE1_MASK_NULL_LOSS:
        ignore_idx_val = 0  # Use 0 as the index to ignore
        logger.info(f"PHASE1_MASK_NULL_LOSS enabled: Using ignore_index={ignore_idx_val} for loss functions")
    ...
    for func, task_key in zip(funcs_train, task_keys, strict=False):
        ...
        logger.debug(f"  Train - Task '{task_key}': {criteria_train[task_key].__class__.__name__}, ignore_index={ignore_idx_val}")
    ...
    for func, task_key in zip(funcs_val, task_keys, strict=False):
        ...
        logger.debug(f"  Val   - Task '{task_key}': {criteria_val[task_key].__class__.__name__}, ignore_index={ignore_idx_val}")
...
def get_loss_function(
    ...
    if apply_class_weights and isinstance(class_weights, dict) and class_weights:
        ...
        try:
            ...
            sorted_weights = [class_weights.get(i, 1.0) for i in range(max_idx + 1)]  # Use 1.0 for missing indices
            weight_tensor = torch.tensor(sorted_weights, dtype=torch.float32)
            logger.debug(f"  Converted class_weights dict to tensor (size {weight_tensor.shape}) for task '{task_key}'")
        except (ValueError, TypeError) as e:
            logger.warning(f"Could not convert class_weights dict to tensor for task '{task_key}': {e}. Using None.")
            weight_tensor = None
    ...
    if loss_type == "CrossEntropyLoss":
        return CrossEntropyLoss(weight=weight_tensor, apply_class_weights=apply_class_weights, ignore_index=ignore_index)

    elif loss_type == "SoftTargetCrossEntropy":
        # Note: SoftTarget CE usually comes from Mixup, class weights might behave differently.
        return SoftTargetCrossEntropy(weight=weight_tensor, apply_class_weights=apply_class_weights)

    elif loss_type == "LabelSmoothingCrossEntropy":
        smoothing = config.MODEL.LABEL_SMOOTHING if hasattr(config.MODEL, "LABEL_SMOOTHING") else 0.1
        return LabelSmoothingCrossEntropy(
            smoothing=smoothing, weight=weight_tensor, apply_class_weights=apply_class_weights, ignore_index=ignore_index, config=config
        )

    elif loss_type == "TaxonomyAwareLabelSmoothing":
        if not task_key:
            raise ValueError("task_key must be provided for TaxonomyAwareLabelSmoothing")
        if taxonomy_matrices is None or task_key not in taxonomy_matrices:
            # Check if smoothing was enabled for this task
            task_idx = config.DATA.TASK_KEYS_H5.index(task_key) if task_key in config.DATA.TASK_KEYS_H5 else -1
            is_enabled = (
                task_idx >= 0
            ...
            if is_enabled:
                error_msg += (
                    " despite smoothing being enabled. Matrix generation might have failed or hierarchy data might be missing/invalid."
                )
            else:
                error_msg += ". Taxonomy smoothing is not enabled for this task in the config."

            logger.error(error_msg)
            raise ValueError(error_msg)

        smoothing_matrix = taxonomy_matrices[task_key]
        logger.info(f"Using taxonomy-aware label smoothing for task '{task_key}', matrix shape: {smoothing_matrix.shape}")
...
def calculate_class_weights(label_counts, config, override_method=None):
    ...
    methods = get_task_specific_config(config.LOSS.GRAD_WEIGHTING.CLASS.METHOD, task_keys, "GRAD_WEIGHTING.CLASS.METHOD")
    ...
    if hasattr(config.LOSS.GRAD_WEIGHTING.CLASS, "SMOOTHING_FACTOR"):
        smoothing_factors = get_task_specific_config(
            config.LOSS.GRAD_WEIGHTING.CLASS.SMOOTHING_FACTOR, task_keys, "GRAD_WEIGHTING.CLASS.SMOOTHING_FACTOR"
        )
    else:
    ...
    if hasattr(config.LOSS.GRAD_WEIGHTING.CLASS, "CAP"):
        caps = get_task_specific_config(config.LOSS.GRAD_WEIGHTING.CLASS.CAP, task_keys, "GRAD_WEIGHTING.CLASS.CAP")
    else:
        # Default value if not provided
    ...
    if hasattr(config.LOSS.GRAD_WEIGHTING.CLASS, "BASE"):
        bases = get_task_specific_config(config.LOSS.GRAD_WEIGHTING.CLASS.BASE, task_keys, "GRAD_WEIGHTING.CLASS.BASE")
    else:
        # Default value if not provided
    ...
    if hasattr(config.LOSS.GRAD_WEIGHTING.CLASS, "LOG_BASE"):
        log_bases = get_task_specific_config(config.LOSS.GRAD_WEIGHTING.CLASS.LOG_BASE, task_keys, "GRAD_WEIGHTING.CLASS.LOG_BASE")
    else:
        # Default value if not provided
    ...
    for i, task in enumerate(task_keys):
        ...
        if counts is None:
            logger.warning(f"Task '{task}' not found in label_counts; using uniform weighting with 1 class.")
            class_weights[task] = {0: 1.0}  # Default for a single class if counts missing
            continue
        ...
        if not isinstance(counts, torch.Tensor):
            try:
                ...
                logger.error(f"Could not convert counts for task '{task}' to tensor: {e}. Skipping weight calculation.")
        ...
        logger.info(f"Task '{task}': Calculating weights using method = '{method_to_use}'")

        if total_samples == 0 or len(counts) == 0:
            logger.warning(f"No samples or classes for task '{task}', using uniform weighting = 1.0")
        ...
        try:
            if method_to_use == "smoothing":
                ...
                task_class_wts = {idx: total_samples / (count.item() + smoothing_factor) for idx, count in enumerate(counts)}
            elif method_to_use == "capping":
                cap = caps[i]
                logger.debug(f"  Cap: {cap}")
                raw_weights = {idx: total_samples / count.item() if count.item() > 0 else 1.0 for idx, count in enumerate(counts)}
                task_class_wts = {idx: min(wt, cap) for idx, wt in raw_weights.items()}
            elif method_to_use == "exponential":
                base = bases[i]
                logger.debug(f"  Base: {base}")
                task_class_wts = {
                    idx: (base ** (total_samples / count.item())) if count.item() > 0 else 1.0 for idx, count in enumerate(counts)
                }
                ...
                for idx, count in enumerate(counts):
                    ...
                    if count_val > 0:
                        ...
                        val = 1.0 / (1 + torch.log(torch.tensor(ratio * (log_base - 1) + 1)) / torch.log(torch.tensor(log_base)))
                ...
                logger.error(f"Unsupported weighting method: '{method_to_use}' for task '{task}'")
                raise ValueError(f"Unsupported method: {method_to_use}")
        except IndexError:
            logger.error(
                f"Index error during weight calculation for task '{task}'. Likely mismatch between config list length and number of tasks."
            )
            # Fallback to uniform weights for this task
            task_class_wts = dict.fromkeys(range(len(counts)), 1.0)
        except Exception as e:
            logger.error(f"Unexpected error calculating weights for task '{task}' with method '{method_to_use}': {e}")
