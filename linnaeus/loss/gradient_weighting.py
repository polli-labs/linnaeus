...
from linnaeus.utils.distributed import get_rank_safely, is_distributed_and_initialized
...
def log_memory_usage(prefix: str, rank: int = 0, force_sync: bool = True, config: Any | None = None, debug_level: bool = False):
    ...
    if not logger.isEnabledFor(logging.DEBUG) and not (debug_level and debug_mem_profiling):
        return
    ...
    if torch.cuda.is_available():
        ...
        if debug_level and debug_mem_profiling:
            ...
            sorted_stats = sorted(tensor_stats.items(), key=lambda x: x[1]["total_mb"], reverse=True)
            top_n = min(5, len(sorted_stats))
            if top_n > 0:
                logger.debug(f"[DEBUG_GRADNORM_MEM][{prefix}] Top {top_n} tensor shapes by memory usage:")
...
class GradientWeighting(nn.Module):
    ...
    def __init__(
        ...
        self.zero_aux_info = getattr(config.LOSS.GRAD_WEIGHTING.TASK, "ZERO_AUX_INFO", zero_aux_info)
        ...
        if task_weighting_type == "gradnorm":
            ...
            if self.rank == 0:
                logger.debug(f"[GradientWeighting] GradNorm => alpha={alpha}, update_interval={update_interval}, zero_aux={zero_aux_info}")
        else:
            # Static weighting
            self.gradnorm = None
            self.task_weights = torch.tensor(init_weights, dtype=torch.float32)
        ...
        if self.rank == 0:
            logger.info(f"[GradientWeighting] tasks={task_keys}, type={task_weighting_type}")
            logger.info(f"[GradientWeighting] init_weights={dict(zip(task_keys, init_weights, strict=False))}")
            if exclude_patterns:
    ...
    def set_model(self, model: nn.Module) -> None:
        ...
        if self.task_weighting_type == "gradnorm":
            ...
            self.backbone_params = [p for name, p in named_params if p.requires_grad and not f.matches(name, p)]

            # Optionally log what was excluded at debug level
            if self.rank == 0:
                logger.info(f"[GradientWeighting.set_model] => Found {len(self.backbone_params)} backbone params")

                if logger.isEnabledFor(logging.DEBUG):
                    from linnaeus.utils.unified_filtering import inspect_gradnorm_filters
    ...
    def forward(
        ...
        if self.gradnorm is not None:
            ...
            norm_w = self._normalize_weights(self.task_weights).to(device=device, dtype=dtype)
    ...
    def update_gradnorm_weights_reforward(
        self, data_batch: Any, criteria: dict[str, nn.Module], amp_enabled: bool = True, ops_schedule=None, current_step=None
    ) -> dict[str, Any]:
        """
        ...
        if self.model is None:
            if self.rank == 0:
                logger.error("[GradNorm] Model not set in GradientWeighting. Cannot update.")
            return {}

        if not self.backbone_params:
            if self.rank == 0:
                logger.error("[GradNorm] Backbone parameters not identified. Cannot compute gradients.")
            return {}
        ...
        gradnorm_use_linear_heads = self.config.LOSS.GRAD_WEIGHTING.TASK.get("USE_LINEAR_HEADS_FOR_GRADNORM_REFORWARD", True)
        if gradnorm_use_linear_heads:
            if self.rank == 0:
                logger.info("[GradNorm] Temporarily switching conditional heads to 'GradNorm mode'")
        ...
        gradnorm_ckpt_flag = bool(self.config.TRAIN.GRADIENT_CHECKPOINTING.ENABLED_GRADNORM_STEPS)
        if self.rank == 0 and gradnorm_ckpt_flag:
            logger.info("[GradNorm] Using gradient checkpointing for re-forward passes.")

        # --- Memory Profiling Setup ---
        reset_peak_memory_stats(self.rank)
        log_memory_usage("GradNorm-Start", self.rank, config=self.config, debug_level=debug_memory)

        if debug_memory and self.rank == 0:
            logger.debug("[DEBUG_GRADNORM_MEM] Starting GradNorm reforward with detailed memory profiling")
            ...
            logger.debug(f"[DEBUG_GRADNORM_MEM] Number of backbone params: {len(self.backbone_params)}")

        # --- Data Preparation ---
        # Unpack data batch, move to GPU, handle aux_info_for_grad
        images, targets_dict, aux_info_orig = (data_batch[0], data_batch[1], data_batch[2])
        ...
        tdict_gpu = {k: v.to(device, non_blocking=True) for k, v in targets_dict.items()}
        aux_info_for_grad = torch.zeros_like(aux_info_orig) if self.zero_aux_info else aux_info_orig
        ...
        if accum_steps > 1 and self.rank == 0:
            logger.info(f"[GradNorm] Using internal accumulation with {accum_steps} steps")
        ...
        for task_idx, tkey in enumerate(self.task_keys):
            reset_peak_memory_stats(self.rank)
            log_memory_usage(
                f"GradNorm-Task-{task_idx + 1}/{len(self.task_keys)}-Start-{tkey}", self.rank, config=self.config, debug_level=debug_memory
            )

            if debug_verbose and self.rank == 0:
                logger.debug(f"[DEBUG_GRADNORM_STEP] === Processing Task: {tkey} ({task_idx + 1}/{len(self.task_keys)}) ===")

            task_aggregated_grads: list[torch.Tensor] | None = None  # Stores sum of grads across sub-batches
            ...
            for s_idx in range(accum_steps):
                ...
                # Sub-batch slicing
                if accum_steps == 1:
                    sb_images, sb_aux_info, sb_targets = (images, aux_info_for_grad, tdict_gpu)
                    start, end = 0, images.size(0)
                else:
                    sub_batch_size = images.size(0) // accum_steps
                    start = s_idx * sub_batch_size
                    end = (s_idx + 1) * sub_batch_size if s_idx < accum_steps - 1 else images.size(0)
                ...
                with autocast(enabled=amp_enabled):
                    outputs_all = self.model(sb_images, sb_aux_info, force_checkpointing=gradnorm_ckpt_flag)
                    log_memory_usage(
                    ...
                    if tkey not in outputs_all:
                        if self.rank == 0:
                            logger.warning(f"[GradNorm] Task {tkey} output not found in model outputs. Skipping task.")
                        skip_task = True
                    ...
                    if debug_verbose and self.rank == 0:
                        logger.debug(f"[DEBUG_GRADNORM_REQ_GRAD] Task {tkey}: out_t requires_grad = {out_t.requires_grad}")
                ...
                if debug_distributed and logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"[Rank {self.rank}] Starting gradient calculation for Task {tkey} - SubBatch {s_idx + 1}/{accum_steps}")
                ...
                try:
                    ...
                    params_to_grad = [p for p in self.backbone_params if p.requires_grad]
                    ...
                    if not partial_loss.requires_grad:
                        ...
                        current_grads_for_sub_batch = [torch.zeros_like(p) for p in params_to_grad]
                    else:
                        ...
                        with autocast(enabled=amp_enabled):
                            ...
                            current_grads_for_sub_batch = [
                                g.clone().detach() if g is not None else torch.zeros_like(p)
                    ...
                    if task_aggregated_grads is None:
                        ...
                        task_aggregated_grads = [g.clone() for g in current_grads_for_sub_batch]
                    else:
                    ...
                    if "out of memory" in str(e).lower():
                        ...
                        logger.error(f"{rank_prefix}[GradNorm] RuntimeError during torch.autograd.grad for Task {tkey}: {e}")
                        raise
                except Exception as e:
                    # Log detailed error for both rank 0 and other ranks if distributed debug enabled
                    rank_prefix = f"[Rank {self.rank}]" if debug_distributed else ""
                    logger.error(f"{rank_prefix}[GradNorm] Unexpected error during torch.autograd.grad for Task {tkey}: {e}")
                    raise
            ...
            if task_aggregated_grads is not None:
                ...
                flat_grads = [g.flatten() for g in task_aggregated_grads if g is not None]
                if flat_grads:
                    # Use FP32 for norm stability
                    aggregated_grad_tensor = torch.cat(flat_grads).detach().to(dtype=torch.float32)

                    # Store for GradNorm calculation
                    grad_tensors[tkey] = aggregated_grad_tensor
                    unweighted_losses[tkey] = torch.tensor(aggregated_loss_val, device=device, dtype=torch.float32)
                    ...
                    if self.rank == 0:
                        logger.warning(f"[GradNorm] Task {tkey}: No valid aggregated gradients found after accumulation.")
            else:
                if self.rank == 0:
                    logger.warning(f"[GradNorm] Task {tkey}: task_aggregated_grads is None. Skipping metrics update.")
            ...
            if is_distributed_and_initialized():
                if self.rank == 0 and (debug_verbose or debug_distributed):
                    logger.debug(f"[DEBUG_DISTRIBUTED] Rank {self.rank} waiting at barrier after task {tkey}")
                dist.barrier()
                if self.rank == 0 and (debug_verbose or debug_distributed):
                    logger.debug(f"[DEBUG_DISTRIBUTED] Rank {self.rank} passed barrier after task {tkey}")
            # ---------------------------------------
        ...
        if not grad_tensors:
            if self.rank == 0:
                logger.warning("[GradNorm] No valid gradients collected for any task. Skipping weight update.")
        else:
            if self.rank == 0 and debug_verbose:
                logger.debug(f"[DEBUG_GRADNORM_STEP] Calling GradNormModule.measure_and_update with {len(grad_tensors)} tasks.")
            ...
            if self.rank == 0 and debug_verbose:
                logger.debug(f"[DEBUG_GRADNORM_STEP] GradNorm update complete. New weights: {self.task_weights.tolist()}")
        ...
        local_vars = locals()
        if accum_steps == 1 and "sb_images" in local_vars and "sb_aux_info" in local_vars and "sb_targets" in local_vars:
        ...
        log_memory_usage("GradNorm-End", self.rank, config=self.config, debug_level=debug_memory)

        if gradnorm_use_linear_heads:
            if self.rank == 0:
                logger.info("[GradNorm] Restoring conditional heads to normal hierarchical mode.")
            for head in self.model.head.values():
                if hasattr(head, "set_gradnorm_mode"):
