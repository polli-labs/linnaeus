...
def filter_log_files(
    log_dir: str, output_file: str, flags: list = None, blacklist_flags: list = None, rank: int = 0, log_type: str = "debug"
):
    """
    ...
    if flags and blacklist_flags:
        raise ValueError("Cannot use both whitelist ('flags') and 'blacklist_flags' simultaneously.")
    ...
    if log_type in ["debug", "both"]:
        ...
        file_patterns.append(f"h5data_debug_log_{rank_pattern}.txt*")  # Include h5data debug logs
    if log_type in ["info", "both"]:
        file_patterns.append(f"info_log_{rank_pattern}.txt*")
        file_patterns.append(f"h5data_info_log_{rank_pattern}.txt*")  # Include h5data info logs
    ...
    if not target_files:
        script_logger.warning(f"No log files found matching patterns: {file_patterns} in {log_dir}")
        return
    ...
    if mode == "whitelist":
        ...
        script_logger.info(f"Whitelist mode: Including logs with prefixes: {include_prefixes}")
    elif mode == "blacklist":
        exclude_prefixes = {create_prefix_from_flag(f) for f in blacklist_flags}
        script_logger.info(f"Blacklist mode: Excluding logs with prefixes: {exclude_prefixes}")
    else:
        ...
        with open(output_path, "w", encoding="utf-8") as outfile:
            for log_file in target_files:
                ...
                if lines_written_from_file > 0:
                    script_logger.info(f"  -> Wrote {lines_written_from_file} lines from {log_file.name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Filter linnaeus logs based on debug flag prefixes.")
    parser.add_argument("log_dir", help="Path to the experiment's log directory.")
    parser.add_argument("-o", "--output", default="filtered_log.txt", help="Output filename (will be placed in log_dir).")
    ...
    filter_group.add_argument("-b", "--blacklist", nargs="+", help="Blacklist: Exclude logs matching these flags.")

    parser.add_argument("-r", "--rank", type=int, default=0, help="Filter logs for a specific rank (default: 0). Use -1 for all ranks.")
    parser.add_argument(
        "-t", "--type", choices=["debug", "info", "both"], default="debug", help="Log file type to process (default: debug)."
    )

    args = parser.parse_args()

    filter_log_files(
        log_dir=args.log_dir, output_file=args.output, flags=args.flags, blacklist_flags=args.blacklist, rank=args.rank, log_type=args.type
    )
