            ...
            for name_dir in group_dir.iterdir():
                ...
                if is_experiment_run(name_dir):
                    try:
                        ...
                        yield Run(
                            path=name_dir.resolve(), project=project_dir.name, group=group_dir.name, name=name_dir.name, timestamp=timestamp
                        )
...
def runs_to_markdown(runs: List[Run], base_dir: Path) -> str:
    ...
    for run in sorted(runs, key=lambda r: r.timestamp, reverse=True):
        ...
        lines.append(f"| {run.project} | {run.group} | {run.name} | {run.timestamp.strftime('%Y-%m-%d %H:%M')} | `{rel_path}` |")
