...
def build_summary(run_path: Path, write_cache: bool = False) -> RunSummary:
    ...
    try:
        ...
        return RunSummary(
            ...
            run_id=run_id,
            config=ExperimentConfig(
                batch_size=0, accumulation_steps=0, aug_pipeline_device="unknown",
                gpu_compile_enabled=False, gpu_compile_mode="unknown", 
                profiler_enabled=False, model_type="unknown", amp_level="unknown",
                num_workers=0
            ),
            profiler_metrics=None,
...
def analyze_profiler_traces(trace_files: List[Path]) -> ProfilerMetrics:
    ...
    for trace_file in trace_files:
        ...
        memory_events = [e for e in events if e.get('cat') == 'gpu_memcpy' or 
                        'memory' in e.get('name', '').lower()]
...
def format_markdown(summary: RunSummary) -> str:
    ...
    lines.extend([
        f"- **Profiler Enabled**: {summary.config.profiler_enabled}",
        ""
    ])
    ...
    if summary.profiler_metrics:
        ...
        lines.extend([
            "## Profiler Metrics",
            "",
            f"❌ Error analyzing {summary.trace_files_count} trace files",
            ""
        ])
    else:
        lines.extend([
