...
def launch(logdir: Path, port: int = 6006, bind_all: bool = False, auto_open: bool = True) -> None:
    """
...
def check_tensorboard_available() -> bool:
    ...
    """
    try:
        subprocess.run(["tensorboard", "--help"], capture_output=True, check=True)
        return True
...
def find_tensorboard_plugins() -> list[str]:
    ...
    try:
        result = subprocess.run(["tensorboard", "--helpfull"], capture_output=True, text=True, check=True)
...
def validate_logdir_structure(logdir: Path) -> dict:
    ...
    for subdir in subdirs:
        ...
        profiler_locations = [subdir / "profiler", subdir / "assets" / "profiler"]
    ...
    if not result["has_profiler_data"]:
        result["recommendations"].append("No profiler traces found. Run experiments with DEBUG.PROFILER.ENABLED: True")
    ...
    if result["valid"] and result["has_profiler_data"]:
        result["recommendations"].append(
            f"Ready for TensorBoard! Found {result['run_count']} runs with {result['profiler_trace_count']} profiler traces."
        )
    elif result["valid"]:
        result["recommendations"].append(f"Directory structure looks good ({result['run_count']} runs), but no profiler data found.")
