...
class Metric:
    ...
    def __init__(self, name: str, init_value: float = 0.0, higher_is_better: bool = True):
        self.name = name
...
class MetricsTracker:
    ...
    def __init__(self, config, subset_maps):
        ...
        self.null_tracking_enabled = getattr(self.config.METRICS, "TRACK_NULL_VS_NON_NULL", False)
        self.null_tracking_tasks = getattr(self.config.METRICS, "NULL_VS_NON_NULL_TASKS", ["taxa_L10"])
        ...
        self.phase_metrics = {
            "train": {
                ...
                "chain_accuracy": Metric("train_chain_accuracy", init_value=0.0, higher_is_better=True),
            ...
            "val": {
                ...
                "chain_accuracy": Metric("val_chain_accuracy", init_value=0.0, higher_is_better=True),
            ...
            "val_mask_meta": {
                ...
                "chain_accuracy": Metric("val_mask_meta_chain_accuracy", init_value=0.0, higher_is_better=True),
                ...
                for tkey in self.null_tracking_tasks:
                    if tkey in self.config.DATA.TASK_KEYS_H5:
                        ...
                        self.phase_null_metrics[phase][tkey] = {
                            "acc1": Metric(f"{phase}_null_acc1_{tkey}", 0.0, higher_is_better=True),
                            "loss": Metric(f"{phase}_null_loss_{tkey}", 1e9, higher_is_better=False),
                        }
                        # Initialize non-null metrics
                        self.phase_non_null_metrics[phase][tkey] = {
                            "acc1": Metric(f"{phase}_non_null_acc1_{tkey}", 0.0, higher_is_better=True),
                            "loss": Metric(f"{phase}_non_null_loss_{tkey}", 1e9, higher_is_better=False),
                        }

        # Partial sums and counts for null/non-null metric accumulation
        self.partial_null_sums = {phase: defaultdict(lambda: defaultdict(float)) for phase in phases}
        self.partial_null_counts = {phase: defaultdict(lambda: defaultdict(int)) for phase in phases}
        self.partial_non_null_sums = {phase: defaultdict(lambda: defaultdict(float)) for phase in phases}
        self.partial_non_null_counts = {phase: defaultdict(lambda: defaultdict(int)) for phase in phases}
        ...
        self.phase_subset_metrics = {phase: self._init_subset_metrics() for phase in phases}
        ...
        self.partial_chain_correct = dict.fromkeys(phases, 0)  # Added for partial chain accuracy
        self.partial_chain_total = dict.fromkeys(phases, 0)  # Added for partial chain accuracy

        # --------------- Partial sums for tasks (acc1, acc3, loss) ---------------
        # We'll accumulate these after each batch, then finalize per epoch.
        self.partial_task_sums = {p: defaultdict(lambda: defaultdict(float)) for p in phases}
        self.partial_task_counts = {p: defaultdict(lambda: defaultdict(int)) for p in phases}
            ...
            for task_key in self.config.DATA.TASK_KEYS_H5:
                ...
                def base_func(out: torch.Tensor, tgt: torch.Tensor):
                    ...
                    if isinstance(out, dict):
                        ...
                        num_classes = tgt.shape[1] if tgt.dim() > 1 else tgt.max().item() + 1
                        tensor_out = None
                        for key, tensor in out.items():
                            if isinstance(tensor, torch.Tensor) and tensor.size(1) == num_classes:
                                tensor_out = tensor
                                break
                ...
                wrapper = SubsetMetricWrapper(base_metric=base_func, subset_types=[subset_type])
    ...
    def update_val_metrics(
        ...
        self._update_phase_batch(phase_name, outputs, targets, loss_components, subset_ids)
    ...
    def _ensure_phase_exists(self, phase: str):
        ...
        logger.info(f"[MetricsTracker] Dynamically creating metrics structures for phase: '{phase}'")

        # Add to phase metrics
        self.phase_metrics[phase] = {
            "loss": Metric(f"{phase}_loss", 1e9, higher_is_better=False),
            "chain_accuracy": Metric(f"{phase}_chain_accuracy", 0.0, higher_is_better=True),
        ...
        if self.null_tracking_enabled:
            ...
            if phase not in self.partial_non_null_sums:
                self.partial_non_null_sums[phase] = defaultdict(lambda: defaultdict(float))
            if phase not in self.partial_non_null_counts:
                self.partial_non_null_counts[phase] = defaultdict(lambda: defaultdict(int))
    ...
    def _update_phase_batch(
        ...
        if self.chain_total[phase] > 0:
            ...
            if phase in self.phase_metrics and "chain_accuracy" in self.phase_metrics[phase]:
        ...
        partial_chain_acc_batch = self._compute_partial_chain_accuracy(sorted_tasks, outputs, targets)
        ...
        if self.partial_chain_total[phase] > 0:
            current_partial_chain_acc = self.partial_chain_correct[phase] / self.partial_chain_total[phase]
            if phase in self.phase_metrics and "partial_chain_accuracy" in self.phase_metrics[phase]:
                # We'll update the value but not the best_value/best_epoch to preserve those metrics
                self.phase_metrics[phase]["partial_chain_accuracy"].value = current_partial_chain_acc

        # 2) Per-task top1/top3 + partial sums
        for tkey in sorted_tasks:
            out_raw = outputs[tkey]  # Either tensor [B, #classes] or dictionary from ConditionalClassifierHead
            ...
            if isinstance(out_raw, dict):
                ...
                num_classes = tgt_t.shape[1] if tgt_t.dim() > 1 else tgt_t.max().item() + 1
        ...
        if self.null_tracking_enabled and "raw_per_sample_losses" in loss_components:
            ...
            try:
                ...
                debug_enabled = check_debug_flag(self.config, "DEBUG.VALIDATION_METRICS")
            except Exception:
                pass
            ...
            for task_key in self.null_tracking_tasks:
                ...
                if target.dim() > 1:  # One-hot encoded
                    is_null_mask = target[:, 0] > 0.5  # Targets with class 0 (null) active
                ...
                if debug_enabled:
                    logger.debug(f"[NULL_METRICS] Task {task_key}: {null_count} null samples, {non_null_count} non-null samples")
                ...
                if null_count > 0:
                    ...
                    if task_key in outputs:
                        ...
                        if isinstance(out_t, dict):
                            for k, v in out_t.items():
                                if isinstance(v, torch.Tensor) and v.shape[1] == target.shape[1]:
                                    out_t = v
                                    break
                            else:
                        ...
                        if debug_enabled:
                            logger.debug(f"[NULL_METRICS] Task {task_key} null acc1: {null_correct}/{null_count} correct")
                    ...
                    if debug_enabled:
                        ...
                        logger.debug(f"[NULL_METRICS] Task {task_key} null loss: {avg_null_loss:.4f}")
                ...
                if non_null_count > 0:
                    ...
                    if task_key in outputs:
                        ...
                        non_null_correct = ((preds == gts) & is_non_null_mask).sum().item()
                        self.partial_non_null_sums[phase][task_key]["acc1"] += non_null_correct
                        self.partial_non_null_counts[phase][task_key]["acc1"] += non_null_count

                        if debug_enabled:
                            logger.debug(f"[NULL_METRICS] Task {task_key} non-null acc1: {non_null_correct}/{non_null_count} correct")

                    # Calculate average loss for non-null samples
                    non_null_loss_sum = per_sample_loss[is_non_null_mask].sum().item()
                    self.partial_non_null_sums[phase][task_key]["loss"] += non_null_loss_sum
                    self.partial_non_null_counts[phase][task_key]["loss"] += non_null_count

                    if debug_enabled:
                        avg_non_null_loss = non_null_loss_sum / non_null_count
                        logger.debug(f"[NULL_METRICS] Task {task_key} non-null loss: {avg_non_null_loss:.4f}")
    ...
    def _compute_chain_accuracy(self, sorted_task_keys, outputs: dict[str, torch.Tensor], targets: dict[str, torch.Tensor]) -> float:
        """
        ...
        ignore_idx = None
        try:
            if hasattr(self.config.TRAIN, "PHASE1_MASK_NULL_LOSS") and self.config.TRAIN.PHASE1_MASK_NULL_LOSS:
                ignore_idx = 0
        except:
        ...
        if rank == 0:
            logger.info(f"[CHAIN_ACCURACY_TRACKER] Computing chain accuracy for {len(sorted_task_keys)} tasks (ignore_index={ignore_idx})")

            # Log out_list types
            for i, output in enumerate(out_list):
                if isinstance(output, dict):
                    logger.info(f"[CHAIN_ACCURACY_TRACKER] Task {i} output is dictionary with keys: {list(output.keys())}")
                elif isinstance(output, torch.Tensor):
                    logger.info(f"[CHAIN_ACCURACY_TRACKER] Task {i} output is tensor with shape: {output.shape}")
                else:
                    logger.info(f"[CHAIN_ACCURACY_TRACKER] Task {i} output has type: {type(output)}")

        chain_acc = compute_chain_accuracy_vectorized(out_list, tgt_list, ignore_index=ignore_idx)  # returns float
    ...
    @staticmethod
    def _compute_partial_chain_accuracy(sorted_task_keys, outputs: dict[str, torch.Tensor], targets: dict[str, torch.Tensor]) -> float:
        """
        ...
        rank = get_rank_safely()
        if rank == 0:
            logger.info(f"[PARTIAL_CHAIN_ACCURACY_TRACKER] Computing partial chain accuracy for {len(sorted_task_keys)} tasks")

        # Use the partial chain accuracy implementation
        partial_chain_acc = compute_partial_chain_accuracy_vectorized(out_list, tgt_list)  # returns float

        if rank == 0:
            logger.info(f"[PARTIAL_CHAIN_ACCURACY_TRACKER] Partial chain accuracy: {partial_chain_acc:.4f}")
    ...
    def finalize_val_epoch(self, epoch: int, avg_epoch_loss: float, mask_meta: bool = False, phase_name: str = None):
        """
    ...
    def _finalize_phase(self, phase: str, epoch: int, avg_epoch_loss: float):
        ...
        if hasattr(self.config, "DEBUG") and hasattr(self.config.DEBUG, "VALIDATION_METRICS"):
            debug_validation = self.config.DEBUG.VALIDATION_METRICS

        if debug_validation:
            logger.debug(f"[{phase}] Starting _finalize_phase for epoch {epoch} with avg_loss={avg_epoch_loss:.4f}")
        ...
        local_chain_correct = torch.tensor(self.chain_correct.get(phase, 0), device="cuda", dtype=torch.float32)
        local_chain_total = torch.tensor(self.chain_total.get(phase, 0), device="cuda", dtype=torch.long)

        # Partial Chain Accuracy
        local_partial_chain_correct = torch.tensor(self.partial_chain_correct.get(phase, 0), device="cuda", dtype=torch.float32)
        local_partial_chain_total = torch.tensor(self.partial_chain_total.get(phase, 0), device="cuda", dtype=torch.long)
        ...
        partial_chain_acc = total_partial_chain_correct / max(1, total_partial_chain_total)
        ...
        if debug_validation and self.rank == 0:
            ...
            logger.debug(f"[{phase}/chain_accuracy] PRE-UPDATE: obj_id={id(chain_metric_obj)}, current_val={chain_metric_obj.value:.4f}")

            # Log partial chain accuracy
            partial_chain_metric_obj = self.phase_metrics[phase]["partial_chain_accuracy"]
            logger.debug(
        ...
        if phase in self.phase_task_metrics:  # Check if task metrics exist for this phase
            for tkey, tdict in self.phase_task_metrics[phase].items():
                # --- Reduce Accuracy Metrics Across Ranks ---
                for metric_name in ["acc1", "acc3"]:
                    # Get local values
                    local_sum = torch.tensor(self.partial_task_sums[phase][tkey].get(metric_name, 0.0), device="cuda")
                    local_count = torch.tensor(self.partial_task_counts[phase][tkey].get(metric_name, 0), device="cuda", dtype=torch.long)
                    ...
                    metric_val = 100.0 * (total_sum / total_count) if total_count > 0 else 0.0
                ...
                local_loss_sum = torch.tensor(self.partial_task_sums[phase][tkey].get("loss", 0.0), device="cuda")
                local_loss_count = torch.tensor(self.partial_task_counts[phase][tkey].get("loss", 0), device="cuda", dtype=torch.long)
                ...
                avg_task_loss = total_loss_sum / total_loss_count if total_loss_count > 0 else 0.0
                ...
                if debug_validation and self.rank == 0:
                    ...
                    logger.debug(f"[{phase}/{tkey}/loss] PRE-UPDATE: obj_id={id(loss_metric_obj)}, current_val={loss_metric_obj.value:.4f}")
            ...
            if debug_validation and self.rank == 0:
                logger.debug(f"[{phase}] No task metrics found in phase_task_metrics to finalize.")
        ...
        if self.null_tracking_enabled:
            ...
            for task_key in self.null_tracking_tasks:
                ...
                if phase not in self.partial_null_sums:
                    if debug_validation and self.rank == 0:
                        logger.debug(f"[{phase}] Creating missing partial_null_sums for phase")
                    self.partial_null_sums[phase] = defaultdict(lambda: defaultdict(float))
                if phase not in self.partial_null_counts:
                    if debug_validation and self.rank == 0:
                        logger.debug(f"[{phase}] Creating missing partial_null_counts for phase")
                    self.partial_null_counts[phase] = defaultdict(lambda: defaultdict(int))

                if task_key in self.partial_null_sums[phase]:
                    # --- Null Accuracy ---
                    local_null_acc_sum = torch.tensor(self.partial_null_sums[phase][task_key].get("acc1", 0.0), device="cuda")
                    local_null_acc_count = torch.tensor(
                        self.partial_null_counts[phase][task_key].get("acc1", 0), device="cuda", dtype=torch.long
                    )

                    # --- Null Loss ---
                    local_null_loss_sum = torch.tensor(self.partial_null_sums[phase][task_key].get("loss", 0.0), device="cuda")
                    local_null_loss_count = torch.tensor(
                        self.partial_null_counts[phase][task_key].get("loss", 0), device="cuda", dtype=torch.long
                    )
                    ...
                    if total_null_acc_count > 0:
                        ...
                        if phase in self.phase_null_metrics and task_key in self.phase_null_metrics[phase] and self.rank == 0:
                            self.phase_null_metrics[phase][task_key]["acc1"].update(null_acc1, epoch)
                    ...
                    if total_null_loss_count > 0:
                        ...
                        if phase in self.phase_null_metrics and task_key in self.phase_null_metrics[phase] and self.rank == 0:
                            self.phase_null_metrics[phase][task_key]["loss"].update(null_loss, epoch)

                # --- Reduce and calculate final NON-NULL metrics ---
                # Safety check to ensure phase exists in partial_non_null_sums
                if phase not in self.partial_non_null_sums:
                    if debug_validation and self.rank == 0:
                        logger.debug(f"[{phase}] Creating missing partial_non_null_sums for phase")
                    self.partial_non_null_sums[phase] = defaultdict(lambda: defaultdict(float))
                if phase not in self.partial_non_null_counts:
                    if debug_validation and self.rank == 0:
                        logger.debug(f"[{phase}] Creating missing partial_non_null_counts for phase")
                    self.partial_non_null_counts[phase] = defaultdict(lambda: defaultdict(int))

                if task_key in self.partial_non_null_sums[phase]:
                    # --- Non-Null Accuracy ---
                    local_non_null_acc_sum = torch.tensor(self.partial_non_null_sums[phase][task_key].get("acc1", 0.0), device="cuda")
                    local_non_null_acc_count = torch.tensor(
                        self.partial_non_null_counts[phase][task_key].get("acc1", 0), device="cuda", dtype=torch.long
                    )

                    # --- Non-Null Loss ---
                    local_non_null_loss_sum = torch.tensor(self.partial_non_null_sums[phase][task_key].get("loss", 0.0), device="cuda")
                    local_non_null_loss_count = torch.tensor(
                        self.partial_non_null_counts[phase][task_key].get("loss", 0), device="cuda", dtype=torch.long
                    )
                    ...
                    if total_non_null_acc_count > 0:
                        non_null_acc1 = 100.0 * (total_non_null_acc_sum / total_non_null_acc_count)
                        if debug_validation and self.rank == 0:
                            logger.debug(
                                f"[{phase}/{task_key}/non_null_acc1] REDUCED: sum={total_non_null_acc_sum}, count={total_non_null_acc_count}, avg={non_null_acc1:.4f}"
                            )
                        if phase in self.phase_non_null_metrics and task_key in self.phase_non_null_metrics[phase] and self.rank == 0:
                            self.phase_non_null_metrics[phase][task_key]["acc1"].update(non_null_acc1, epoch)

                    # Loss
                    if total_non_null_loss_count > 0:
                        non_null_loss = total_non_null_loss_sum / total_non_null_loss_count
                        if debug_validation and self.rank == 0:
                            logger.debug(
                        ...
                        if phase in self.phase_non_null_metrics and task_key in self.phase_non_null_metrics[phase] and self.rank == 0:
                            self.phase_non_null_metrics[phase][task_key]["loss"].update(non_null_loss, epoch)

        # --- (Optional) Store subset snapshot (only on rank 0) ---
        if phase in ["train", "val"] and self.rank == 0:  # e.g. only store a snapshot for non-mask phases, only on rank 0
            subset_snapshot = {}
            for subset_type, wrappers_by_task in self.phase_subset_metrics[phase].items():
                merged = {}
                for tkey, subset_wrapper in wrappers_by_task.items():
            ...
            if subset_snapshot:
                ...
                if debug_validation:
                    logger.debug(f"[{phase}] Stored subset snapshot with {len(subset_snapshot)} subset types")

        # Final logging before reset if debug is enabled (only on rank 0)
        if debug_validation and self.rank == 0:
            logger.debug(f"[{phase}] Metrics finalized, preparing to reset accumulators.")
        # ----------------------------------------------------
    ...
    def get_best_epoch(self, phase: str, metric_name: str) -> int:
        ...
        """
        if phase not in self.phase_metrics or metric_name not in self.phase_metrics[phase]:
            return 0
    ...
    def update_top_n_epochs(self, epoch: int):
        ...
        self.top_n_epochs_data = [(e, v, m) for e, v, m in self.top_n_epochs_data if e != epoch]
        ...
        for comp_name, comp_val in loss_dict.items():
            if isinstance(comp_val, dict):
                for sub_name, sub_val in comp_val.items():
                    val_tensor = torch.tensor(sub_val, dtype=torch.float32) if isinstance(sub_val, float) else sub_val
                    self.loss_components[comp_name][sub_name] += val_tensor
            else:
                val_tensor = torch.tensor(comp_val, dtype=torch.float32) if isinstance(comp_val, float) else comp_val
                self.loss_components[comp_name]["value"] += val_tensor

    def update_task_weights(self, task_weights: dict[str, float], subset_weights: dict[str, float]):
        """
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        historical_null_masking_stats = getattr(self, "historical_null_masking_stats", [])
        ...
        state = {
            ...
            "actual_meta_valid_pct": {
                phase: {
                    ...
                    else {"val": avg_meter.val, "avg": avg_meter.avg, "sum": avg_meter.sum, "count": avg_meter.count}
    ...
    def load_state_dict(self, state: dict[str, Any]):
        ...
        if "phase_metrics" in state and "val_mask" in state["phase_metrics"]:
            ...
            if "val_mask_meta" not in state["phase_metrics"]:
                state["phase_metrics"]["val_mask_meta"] = state["phase_metrics"]["val_mask"]

        if "phase_task_metrics" in state and "val_mask" in state["phase_task_metrics"]:
            # Create val_mask_meta entry if it doesn't exist
            if "val_mask_meta" not in state["phase_task_metrics"]:
                state["phase_task_metrics"]["val_mask_meta"] = state["phase_task_metrics"]["val_mask"]

        # Handle chain_data backward compatibility
        if "chain_data" in state:
            if "chain_correct" in state["chain_data"] and "val_mask" in state["chain_data"]["chain_correct"]:
                if "val_mask_meta" not in state["chain_data"]["chain_correct"]:
                    state["chain_data"]["chain_correct"]["val_mask_meta"] = state["chain_data"]["chain_correct"]["val_mask"]

            if "chain_total" in state["chain_data"] and "val_mask" in state["chain_data"]["chain_total"]:
                if "val_mask_meta" not in state["chain_data"]["chain_total"]:
                    state["chain_data"]["chain_total"]["val_mask_meta"] = state["chain_data"]["chain_total"]["val_mask"]
                ...
                for task_key, subm in tdict.items():
                    if task_key not in self.phase_null_metrics[phase]:
                        self.phase_null_metrics[phase][task_key] = {
                            "acc1": Metric(f"{phase}_null_acc1_{task_key}", 0.0, higher_is_better=True),
                            "loss": Metric(f"{phase}_null_loss_{task_key}", 1e9, higher_is_better=False),
                        }
                    for stat_name, sm_state in subm.items():
                        self.phase_null_metrics[phase][task_key][stat_name].load_state_dict(sm_state)
                ...
                for task_key, subm in tdict.items():
                    if task_key not in self.phase_non_null_metrics[phase]:
                        self.phase_non_null_metrics[phase][task_key] = {
                            "acc1": Metric(f"{phase}_non_null_acc1_{task_key}", 0.0, higher_is_better=True),
                            "loss": Metric(f"{phase}_non_null_loss_{task_key}", 1e9, higher_is_better=False),
                        }
                    for stat_name, sm_state in subm.items():
                        self.phase_non_null_metrics[phase][task_key][stat_name].load_state_dict(sm_state)
        ...
        if "rank" in state:
            ...
            # For backward compatibility with checkpoints that don't have this field
            self.rank = get_rank_safely()
            logger.info(f"Initialized rank={self.rank} from get_rank_safely() during state_dict loading")
        ...
        if "top_n_epochs_data" in state:
            ...
            logger.info("Initialized empty top_n_epochs_data for backward compatibility")
        ...
        if "actual_meta_valid_pct" in state:
            for phase, comp_dict in state["actual_meta_valid_pct"].items():
                ...
                for comp_name, meter_state in comp_dict.items():
                    ...
                    if hasattr(self.actual_meta_valid_pct[phase][comp_name], "load_state_dict"):
                        self.actual_meta_valid_pct[phase][comp_name].load_state_dict(meter_state)
                    else:
    ...
    def get_wandb_metrics(self) -> dict[str, float]:
        ...
        if debug_validation_metrics:
            ...
            if rank == 0:
                logger.debug(f"[DEBUG_VALIDATION_METRICS] get_wandb_metrics called, available phases: {list(self.phase_metrics.keys())}")
                logger.debug(f"[DEBUG_VALIDATION_METRICS] phase_task_metrics phases: {list(self.phase_task_metrics.keys())}")
        ...
        if "prefetch_times" in self.metrics and self.metrics["prefetch_times"]:
            metrics["pipeline/timing/prefetch_times"] = self.metrics["prefetch_times"][-1]
        if "preprocess_times" in self.metrics and self.metrics["preprocess_times"]:
            metrics["pipeline/timing/preprocess_times"] = self.metrics["preprocess_times"][-1]
        ...
        if "val_mask_meta" in self.phase_metrics and "loss" in self.phase_metrics["val_mask_meta"]:
            metrics["core/valMask_loss"] = self.phase_metrics["val_mask_meta"]["loss"].value

        # b) chain accuracies
        metrics["core/train_chain_acc"] = self.phase_metrics["train"]["chain_accuracy"].value
        metrics["core/train_chain_acc_pct"] = self.phase_metrics["train"]["chain_accuracy"].value * 100.0
        metrics["core/val_chain_acc"] = self.phase_metrics["val"]["chain_accuracy"].value
        metrics["core/val_chain_acc_pct"] = self.phase_metrics["val"]["chain_accuracy"].value * 100.0
        metrics["core/valMask_chain_acc"] = self.phase_metrics["val_mask_meta"]["chain_accuracy"].value
        metrics["core/valMask_chain_acc_pct"] = self.phase_metrics["val_mask_meta"]["chain_accuracy"].value * 100.0

        # c) partial chain accuracies (if available)
        if "partial_chain_accuracy" in self.phase_metrics["train"]:
            metrics["core/train_partial_chain_acc"] = self.phase_metrics["train"]["partial_chain_accuracy"].value
            metrics["core/train_partial_chain_acc_pct"] = self.phase_metrics["train"]["partial_chain_accuracy"].value * 100.0
        if "partial_chain_accuracy" in self.phase_metrics["val"]:
            metrics["core/val_partial_chain_acc"] = self.phase_metrics["val"]["partial_chain_accuracy"].value
            metrics["core/val_partial_chain_acc_pct"] = self.phase_metrics["val"]["partial_chain_accuracy"].value * 100.0
        if "partial_chain_accuracy" in self.phase_metrics["val_mask_meta"]:
            metrics["core/valMask_partial_chain_acc"] = self.phase_metrics["val_mask_meta"]["partial_chain_accuracy"].value
            metrics["core/valMask_partial_chain_acc_pct"] = self.phase_metrics["val_mask_meta"]["partial_chain_accuracy"].value * 100.0

        # c) per-task top1 accuracy for val, val_mask_meta
        if "val" in self.phase_task_metrics and "val_mask_meta" in self.phase_task_metrics:
            for tkey in self.phase_task_metrics["val"].keys():
                ...
                # top1 val_mask_meta:
                acc1_val_mask = self.phase_task_metrics["val_mask_meta"][tkey]["acc1"].value
                ...
                if phase in self.phase_null_metrics:
                    for task_key in self.null_tracking_tasks:
                        if task_key in self.phase_null_metrics[phase]:
                            ...
                            metrics[f"{phase}/null/acc1_{task_key}"] = self.phase_null_metrics[phase][task_key]["acc1"].value
                            metrics[f"{phase}/null/loss_{task_key}"] = self.phase_null_metrics[phase][task_key]["loss"].value

                            # Add to core metrics for easier visibility
                            metrics[f"core/{phase}_null_acc1/{task_key}"] = self.phase_null_metrics[phase][task_key]["acc1"].value
                            metrics[f"core/{phase}_null_loss/{task_key}"] = self.phase_null_metrics[phase][task_key]["loss"].value

                if phase in self.phase_non_null_metrics:
                    for task_key in self.null_tracking_tasks:
                        if task_key in self.phase_non_null_metrics[phase]:
                            # Add non-null metrics to wandb
                            metrics[f"{phase}/non_null/acc1_{task_key}"] = self.phase_non_null_metrics[phase][task_key]["acc1"].value
                            metrics[f"{phase}/non_null/loss_{task_key}"] = self.phase_non_null_metrics[phase][task_key]["loss"].value

                            # Add to core metrics for easier visibility
                            metrics[f"core/{phase}_non_null_acc1/{task_key}"] = self.phase_non_null_metrics[phase][task_key]["acc1"].value
                            metrics[f"core/{phase}_non_null_loss/{task_key}"] = self.phase_non_null_metrics[phase][task_key]["loss"].value
        ...
        if hasattr(self, "null_masking_stats") and self.null_masking_stats:
            if debug_null_masking:
                rank = getattr(self, "rank", 0)
                if rank == 0:
                    logger.debug(f"[DEBUG_NULL_STATS_WANDB] Adding null masking stats to wandb metrics: {self.null_masking_stats}")
            ...
            if "inclusion_percentage" in self.null_masking_stats:
                metrics["core/null_inclusion_pct"] = self.null_masking_stats["inclusion_percentage"]

            if debug_null_masking:
                rank = getattr(self, "rank", 0)
                if rank == 0:
                    logger.debug(f"[DEBUG_NULL_STATS_WANDB] Added {len(self.null_masking_stats)} null masking metrics to wandb")
        ...
        try:
            ...
            debug_validation_metrics = check_debug_flag(self.config, "DEBUG.VALIDATION_METRICS")
        except Exception:
            pass
        ...
        if debug_validation_metrics:
            ...
            if rank == 0:
                ...
                logger_internal = get_main_logger()  # Avoid conflict with module-level logger
                ...
                for p_key, c_dict in self.actual_meta_valid_pct.items():
                    ...
                    for comp_n, avg_m in c_dict.items():
                        logger_internal.debug(f"    Component '{comp_n}': avg={avg_m.avg:.2f}, count={avg_m.count} (id: {id(avg_m)})")
        # --- End logging ---
            ...
            for comp_name, avg_meter in comp_dict.items():
                ...
                if debug_validation_metrics:
                    ...
                    if rank == 0:
                        ...
                        logger = get_main_logger()
                        logger.debug(f"[META_STATS_WANDB] Added metric {metric_key}={avg_meter.avg} to wandb metrics")
    ...
    def update_gradnorm_metrics(self, gradnorm_metrics: dict[str, Any]) -> None:
        ...
        """
        # Check if debugging is enabled
        debug_verbose = getattr(self.config.DEBUG.LOSS, "VERBOSE_GRADNORM_LOGGING", False)

        rank = getattr(self, "rank", 0)  # Get rank safely
        if rank == 0 and debug_verbose:
            logger.debug(f"[DEBUG_GRADNORM_TRACKER_UPDATE] Received gradnorm_metrics: {gradnorm_metrics}")
            logger.debug(f"[DEBUG_GRADNORM_TRACKER_UPDATE] self.gradnorm_metrics BEFORE update: {getattr(self, 'gradnorm_metrics', {})}")
        ...
        if rank == 0 and debug_verbose:
            logger.debug(f"[DEBUG_GRADNORM_TRACKER_UPDATE] self.gradnorm_metrics AFTER update: {self.gradnorm_metrics}")
    ...
    def update_null_masking_stats(self, null_stats: dict[str, Any]) -> None:
        ...
        if rank == 0 and debug_null_masking:
            logger.debug(f"[DEBUG_NULL_STATS_UPDATE] update_null_masking_stats called with stats: {null_stats}")
        ...
        if not hasattr(self, "null_masking_stats"):
            ...
            if rank == 0 and debug_null_masking:
                logger.debug("[DEBUG_NULL_STATS_UPDATE] Initialized self.null_masking_stats")

        # Update the stats
        old_stats = self.null_masking_stats.copy() if hasattr(self, "null_masking_stats") else {}
        ...
        if rank == 0 and debug_null_masking:
            ...
            logger.debug(f"[DEBUG_NULL_STATS_UPDATE]   - After update: {self.null_masking_stats}")

        # Optionally maintain history
        if not hasattr(self, "historical_null_masking_stats"):
            self.historical_null_masking_stats = []
            if rank == 0 and debug_null_masking:
                logger.info("[NULL_MASKING_DEBUG] Initialized self.historical_null_masking_stats")

        self.historical_null_masking_stats.append(null_stats.copy())

        # Limit history size to avoid memory issues
        if len(self.historical_null_masking_stats) > 100:
            self.historical_null_masking_stats = self.historical_null_masking_stats[-100:]

        if rank == 0 and debug_null_masking:
            logger.info(f"[NULL_MASKING_DEBUG] Historical null masking stats now has {len(self.historical_null_masking_stats)} entries")

    def update_schedule_values(self, meta_mask_prob: float, mixup_prob: float, mixup_group: str, epoch: int) -> None:
        """
        ...
        self.historical_schedule_values.append(
            {"epoch": epoch, "meta_mask_prob": float(meta_mask_prob), "mixup_prob": float(mixup_prob), "mixup_group": str(mixup_group)}
        )
    ...
    def update_actual_meta_stats(self, phase: str, stats_dict: dict):
        ...
        if debug_validation_metrics:
            ...
            if rank == 0:
                ...
                logger = get_main_logger()
                logger.debug(f"[META_STATS_TRACKER] update_actual_meta_stats called with phase={phase}, stats_dict={stats_dict}")

                # --- Log content and ID of received stats_dict ---
                logger.debug(f"[META_STATS_TRACKER_RECEIVED] ID of received stats_dict: {id(stats_dict)}")
                ...
                if isinstance(stats_dict, dict):  # Ensure it's a dict before iterating
                    for k_rec, v_rec in stats_dict.items():
                        logger.debug(f"[META_STATS_TRACKER_RECEIVED]   - Received '{k_rec}': {v_rec}")
                        logged_items += 1
                        if logged_items >= 5:
                            break  # Log up to 5 items
                else:
                    logger.debug(f"[META_STATS_TRACKER_RECEIVED]   - Received stats_dict is not a dict, type: {type(stats_dict)}")
        ...
        if not stats_dict:  # Skip if empty (e.g., from non-rank 0)
            if debug_validation_metrics:
                ...
                logger = get_main_logger()
                logger.debug("[META_STATS_TRACKER] Skipping update - stats_dict is empty")
            return
        ...
        if phase not in self.actual_meta_valid_pct:
            if debug_validation_metrics:
                ...
                logger = get_main_logger()
                logger.debug(f"[META_STATS_TRACKER] Phase {phase} not in actual_meta_valid_pct, calling _ensure_phase_exists")
            self._ensure_phase_exists(phase)
        ...
        for comp_name, pct_value in stats_dict.items():
            if comp_name not in self.actual_meta_valid_pct[phase]:
                ...
                if debug_validation_metrics:
                    ...
                    logger = get_main_logger()
                    logger.debug(f"[META_STATS_TRACKER] Initialized AverageMeter for {comp_name} in phase {phase}")

            # Update the percentage value
            # Pass owner_info for targeted AverageMeter debugging
            owner_tag = f"actual_meta_stats_meter_{phase}_{comp_name}"
            self.actual_meta_valid_pct[phase][comp_name].update(pct_value, owner_info=owner_tag, config=self.config)
        ...
        if debug_validation_metrics:
            ...
            logger = get_main_logger()
            logger.debug(f"[META_STATS_TRACKER] Updated actual_meta_valid_pct for phase {phase}: {self.actual_meta_valid_pct[phase]}")
            logger.debug(
            ...
            for cn, am in self.actual_meta_valid_pct[phase].items():
                logger.debug(f"  '{cn}': avg={am.avg:.2f} (count: {am.count}, id: {id(am)})")
    ...
    def dump_metrics_state(self, phase: str = None, log_level: str = "INFO") -> dict[str, Any]:
        """
        ...
        for p in phases_to_dump:
            ...
            for metric_name, metric_obj in self.phase_metrics[p].items():
                ...
                log_method(f"  - {metric_name}: current={value:.4f}, best={best:.4f} (epoch {best_epoch})")
                phase_summary[metric_name] = {"current": value, "best": best, "best_epoch": best_epoch}
            ...
            if p in self.phase_task_metrics:
                ...
                for task_key, metrics in self.phase_task_metrics[p].items():
                    ...
                    for metric_name, metric_obj in metrics.items():
                        ...
                        best_epoch = metric_obj.best_epoch
                        log_method(f"    - {metric_name}: current={value:.4f}, best={best:.4f} (epoch {best_epoch})")
                        task_summary[task_key][metric_name] = {"current": value, "best": best, "best_epoch": best_epoch}
            ...
            if p in self.partial_task_sums:
                ...
                for task_key, metric_dict in self.partial_task_sums[p].items():
                    ...
                    for metric_name, sum_val in metric_dict.items():
                        ...
                        avg = sum_val / max(1, count_val) if metric_name == "loss" else 100.0 * sum_val / max(1, count_val)
                        log_method(f"    - {metric_name}: sum={sum_val:.4f}, count={count_val}, avg={avg:.4f}")
        ...
        for p in phases_to_dump:
            if p in self.chain_correct:
                ...
                log_method(f"Chain accumulation for '{p}': correct={correct:.1f}, total={total}, acc={chain_acc:.4f}")
                summary[f"{p}_chain"] = {"correct": float(correct), "total": int(total), "accuracy": float(chain_acc)}
