...
class LinnaeusInferenceHandler:
    ...
    def load_from_artifacts(
        ...
        base_path = (
            Path(artifacts_base_dir) if artifacts_base_dir else Path(cfg.inference_options.artifacts_source_uri or config_path.parent)
        )
    ...
    def info(self) -> ModelInformation:
        predicted_levels = [self._get_rank_level_from_linnaeus_task_key(key) for key in self.config.model.model_task_keys_ordered]
