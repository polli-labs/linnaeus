...
def train_one_epoch(
    ...
    inner_accum_count = 0
    total_loss_accum_for_epoch_avg = 0.0  # Accumulates unscaled total_loss for epoch average
    ...
    if rank == 0 and (check_debug_flag(config, "DEBUG.DATALOADER") or check_debug_flag(config, "DEBUG.SCHEDULING")):
        logger.debug(f"[train_one_epoch] Dataloader length for ETA calculation: {dataloader_len} batches")

    if rank == 0 and accumulation_steps > 1:
        logger.info(f"[train_one_epoch] Using gradient accumulation with {accumulation_steps} steps")
    ...
    try:
        ...
        for idx, batch_data in enumerate(data_loader):
            if batch_data is STOP_SENTINEL:  # Check for sentinel from dataset
                logger.info(f"[train_one_epoch] Received STOP_SENTINEL from dataloader. Ending epoch {epoch}.")
                break
            ...
            if accumulation_steps > 1 and inner_accum_count < accumulation_steps and rank == 0:
                if ops_schedule.should_log_to_console() or idx % max(1, dataloader_len // 10) == 0:
                    logger.info(
            ...
            if inner_accum_count >= accumulation_steps:
                ...
                step_for_gradnorm_check = training_progress.global_step  # Step *before* potential optimizer step
                is_gradnorm_update_step = ops_schedule.should_update_gradnorm(step_for_gradnorm_check)

                if is_gradnorm_update_step and not gradnorm_run_this_optimizer_step:
                    if rank == 0:
                        logger.info(f"[GradNorm] Triggered at global_step {step_for_gradnorm_check}. Clearing VRAM before re-forward.")

                    # Store current model checkpointing state
                    current_model_ckpt_state_before_gn = getattr(model_to_set_checkpoint_flag, "use_checkpoint", False)
                    # Set GradNorm specific checkpointing
                    gradnorm_specific_ckpt_flag = bool(config.TRAIN.GRADIENT_CHECKPOINTING.ENABLED_GRADNORM_STEPS)
                    if hasattr(model_to_set_checkpoint_flag, "use_checkpoint"):
                        model_to_set_checkpoint_flag.use_checkpoint = gradnorm_specific_ckpt_flag
                        if rank == 0 and debug_training_loop:
                            logger.debug(
                    ...
                    temp_loss_comp_for_gn_log = loss_components  # Save for potential logging after GN
                    ...
                    if hasattr(model_to_set_checkpoint_flag, "use_checkpoint"):
                        model_to_set_checkpoint_flag.use_checkpoint = current_model_ckpt_state_before_gn
                        if rank == 0 and debug_training_loop:
                            logger.debug(
                ...
                batch_loss_dict_for_log = {
                    "total": float(loss_components.get("total", 0.0)),  # Use unscaled loss for logging
        ...
        avg_loss_for_epoch = (total_loss_accum_for_epoch_avg / total_samples_for_epoch_avg) if total_samples_for_epoch_avg > 0 else 0.0
        if rank == 0:
            logger.info(
        ...
        if hasattr(model_to_set_checkpoint_flag, "use_checkpoint"):
            ...
            if rank == 0 and debug_training_loop:
                logger.debug(f"[GC_SETTING train_one_epoch] Restored model.use_checkpoint to {backup_use_ckpt_normal} at end of epoch.")
        if rank == 0:
            log_memory_usage(f"Epoch {epoch} End", rank, config=config, debug_level=check_debug_flag(config, "DEBUG.LOSS.GRADNORM_MEMORY"))
